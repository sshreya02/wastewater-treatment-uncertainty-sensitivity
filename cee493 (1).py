...
input_file = 'Bardenpho run 6.xlsx - Sheet1.csv'

# Load the CSV file
data = pd.read_csv(input_file)

# Initialize the design list and output dictionary
Bardenpho_design_comb = []
Bardenpho_effluent = {}

# Extract every 20th row
for i, row in data.iloc[100::100].iterrows():
    do_aerobic1 = float(row['do_aerobic1'])
    was = float(row['was'])
    methanol = float(row['methanol'])
    bod = float(row['cbod5_clar'])
    nitrogen = float(row['total_n_clar'])

    # Append the selected data to the design combo list
    Bardenpho_design_comb.append((do_aerobic1, was, methanol))

    # Add to output dictionary
    j = len(Bardenpho_design_comb) - 1
    Bardenpho_effluent[Bardenpho_design_comb[j]] = ((bod, nitrogen))

# Print the extracted data to check
print(Bardenpho_design_comb[:5])
print(Bardenpho_effluent)

import pandas as pd
import os

# Construct the file path
input_file = 'MLE run 1.xlsx - Sheet1.csv'
...
# Extract every 20th row
for i, row in data.iloc[20::20].iterrows():
    do_aerobic1 = float(row['do_aerobic'])
    internal_return = float(row['internal_return'])
    methanol = float(row['methanol'])
    bod = float(row['cbod5_dnf'])
    nitrogen = float(row['total_n_dnf'])

    # Append the selected data to the design combo list
    MLE_design_comb.append((do_aerobic1, internal_return, methanol))

    # Add to output dictionary
    j = len(MLE_design_comb) - 1
    MLE_effluent[MLE_design_comb[j]] = ((bod, nitrogen))

# Print the extracted data to check
print(MLE_design_comb[:5])
print(MLE_effluent)
...
# Pump parameters
r = 1 # Recycle ratio for RAS. Varies 0.5-1.5.
...
def pump(pump_config):
  if pump_config == 'MLE':
    ...
    r_pump_power = pump_power(Q, r)
    was_pump_power = pump_power(WAS, SG_sludge)
    return round(ir_pump_power,2), round(r_pump_power,2), round(was_pump_power,2)
...
def calculate_impacts(data, effluent, config_type):
    total_hours = 24 * 365
    n2o_emission_factor = unit_impacts['n2O_emission_M'] if config_type == 'MLE' else unit_impacts['n2O_emission_B']

    N_eff, BOD_eff = effluent

    construction_impact = {
        'wall_concrete': data['V_c_w'] * unit_impacts['wall_concrete'] / project_lifetime,
        'slab_concrete': data['A_slab'] * unit_impacts['slab_concrete'] / project_lifetime,
        'pipe': data['pipe'] * unit_impacts['pipe'] / project_lifetime,
        'dnf_filter_media': data.get('filter_media', 0) * density_of_filter_media * unit_impacts['dnf_media'] / project_lifetime,
        'dnf_supporting_media': data.get('filter_support', 0) * density_of_supporting_media * unit_impacts['dnf_media'] / project_lifetime,
        'transport': ((data['V_c_w'] + data['V_c_s']) * density_of_concrete + data['M_steel'] * density_of_steel) * unit_impacts['transport'] * transportation_distance  / (project_lifetime * 1000)
    }
...
for design in Bardenpho_design_comb:
  ...
  for key in Bardenpho:
    ...
    elif key == 'pumps': Bardenpho[key]['ir_power'], Bardenpho[key]['r_power'], Bardenpho[key]['WAS_power']  = pump('Bardenpho')
    elif key == 'blowers':
      RO = DO * Q/ (24 * 1000) # Required dissolved oxygen, converted to kg/hr.
      Q_air_rx = 6 * RO/SOTE # Required airflow rate, m3/s.
...
for design in MLE_design_comb:
    ...
    effluent = MLE_effluent[design]
    MLE_impacts = calculate_impacts(amounts, effluent, 'MLE')
...
for design in Bardenpho_design_comb:
    DO, WAS, Q_methanol = design
    effluent = Bardenpho_effluent[design]
    amounts = total_amounts['Bardenpho'][design]
    Bardenpho_impacts = calculate_impacts(amounts, effluent, 'Bardenpho')
...
ax1.set_xlabel('Total Impact (Million kg CO2-eq/year)')
plt.tight_layout()
plt.show()

# Create scatter plot for Bardenpho
...
ax2.set_xlabel('Total Impact (Million kg CO2-eq/year)')
plt.tight_layout()
plt.show()

min(mle_impacts)

# Create a scatter plot for both MLE and Bardenpho on one graph
fig, ax = plt.subplots(figsize=(8, 6))

# Plot MLE points
ax.scatter(mle_impacts, mle_costs, color='blue', label='MLE')

# Plot Bardenpho points
ax.scatter(bardenpho_impacts, bardenpho_costs, color='green', label='Bardenpho')

# Add title, labels, and legend
ax.set_title('Cost vs Impact for MLE and Bardenpho')
ax.set_ylabel('Total Cost (Thousand USD/year)')
ax.set_xlabel('Total Impact (Million kg CO2-eq/year)')
ax.legend()
...
# Sample data (replace mle_impacts, mle_costs, bardenpho_impacts, bardenpho_costs with actual data)
# mle_impacts = np.array([...])
# mle_costs = np.array([...])
# bardenpho_impacts = np.array([...])
# bardenpho_costs = np.array([...])
...
# Convex Hull for MLE
mle_points = np.column_stack((mle_impacts, mle_costs))
mle_hull = ConvexHull(mle_points)
mle_hull_points = mle_points[mle_hull.vertices]
ax.fill(mle_hull_points[:, 0], mle_hull_points[:, 1], color='blue', alpha=0.5, label='MLE Design Space')

# Convex Hull for Bardenpho
bardenpho_points = np.column_stack((bardenpho_impacts, bardenpho_costs))
bardenpho_hull = ConvexHull(bardenpho_points)
bardenpho_hull_points = bardenpho_points[bardenpho_hull.vertices]
ax.fill(bardenpho_hull_points[:, 0], bardenpho_hull_points[:, 1], color='green', alpha=0.5, label='Bardenpho Design Space')

# Plot all points for MLE
ax.scatter(mle_points[:, 0], mle_points[:, 1], color='blue', label='MLE Designs')

# Plot all points for Bardenpho
ax.scatter(bardenpho_points[:, 0], bardenpho_points[:, 1], color='green', label='Bardenpho Designs')
